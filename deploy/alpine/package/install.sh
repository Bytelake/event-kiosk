#!/usr/bin/env bash
#
# Install Event Kiosk from a pre-built release package (Alpine Linux).
#
# Usage:
#   tar -xzf event-kiosk-alpine-*.tar.gz
#   cd event-kiosk-alpine-*
...
alpine_install_packages false "${WITH_DISPLAY}" "${DISPLAY_MODE}"
alpine_install_node
...
if [[ "${WITH_DISPLAY}" == "true" ]]; then
  log "Installing Electron shell (system electron via apk)..."
...
alpine_install_services "${DISPLAY_MODE}" "${WITH_DISPLAY}" "${INSTALL_DIR}"

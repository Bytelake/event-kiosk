#!/usr/bin/env bash
# Build Alpine Linux release package (must run on Alpine/musl):
#   npm run package:alpine [-- amd64|arm64] [--pre-release[=label]]
set -euo pipefail
...
if [[ -f /etc/os-release ]]; then
  ...
  if [[ "${ID:-}" != "alpine" ]]; then
    echo "[package:alpine] WARNING: not running on Alpine (ID=${ID:-unknown})." >&2
    echo "[package:alpine] Build on Alpine for musl-compatible binaries." >&2
  fi
fi
...
PACKAGE_NAME="event-kiosk-alpine-${ARCH_LABEL}-${VERSION_LABEL}"
ARCHIVE="${OUT_DIR}/${PACKAGE_NAME}.tar.gz"

log() { echo "[package:alpine] $*"; }
...
rm -rf "${PACKAGE_ROOT}"
mkdir -p "${PACKAGE_ROOT}/web" "${PACKAGE_ROOT}/shell/dist" "${PACKAGE_ROOT}/bin" \
  "${PACKAGE_ROOT}/systemd" "${PACKAGE_ROOT}/openrc" "${PACKAGE_ROOT}/lib"
...
cp "${ROOT}/deploy/common/openrc/"* "${PACKAGE_ROOT}/openrc/"
...
cp "${ROOT}/deploy/alpine/package/lib/helpers.sh" "${PACKAGE_ROOT}/lib/helpers.sh"

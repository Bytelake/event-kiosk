...
After install, **Admin → Settings → Display output** can sleep HDMI on a schedule or after idle so the monitor can power down while the PC stays on. Restart `kiosk-display` once after updating so display-power permissions are applied.
